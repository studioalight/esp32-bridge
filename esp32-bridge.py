...
async def flash_batch(files, port, baudrate, chip='esp32p4', reset_after=True):
    ...
    try:
        ...
        # FIXME: Progress percentage parsing shows 0% because esptool uses \r to update
        # the same line, and the client captures partial output. The carriage return
        # handling above should work but doesn't in practice. Low priority - flash works,
        # just progress display is inaccurate. Would need buffering or different approach.
        ...
        while True:
            ...
            if text:
                ...
                # FIXME: Progress parsing - see note above
                if '%' in text:
                    await broadcast(json.dumps({
                        'type': 'flash_batch', 'status': 'progress',
                        'pct': 0,  # Always 0 due to parsing issue
                        'line': text
                    }))
